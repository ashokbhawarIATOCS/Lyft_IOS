...
class SignUpVC: UIViewController,UITextFieldDelegate,CountryPickerTableViewDelegate, UIPickerViewDelegate, UIPickerViewDataSource {
...
//    @IBOutlet weak var lblEnterDOBErrorMsg: UILabel!
    ...
    @IBOutlet weak var btnPasswordCheck: UIButton!
    @IBOutlet weak var lblConfirmPassword: UILabel!
    @IBOutlet weak var txtConfirmPassword: FormTextField!
    @IBOutlet weak var lblConfirmPasswordErrorMsg: UILabel!
    @IBOutlet weak var btnConfirmPasswordCheck: UIButton!
...
//        updateLocationInfo()
        self.navigationItem.title = Constants.signUpTitle.localize()
...
//        self.navigationItem.hidesBackButton = true
        ...
        txtEmergencyPhoneNumber.addTarget(self, action: #selector(self.textFieldDidChange(_:)),
                           for: UIControl.Event.editingChanged)
        btnPasswordCheck.setImage(UIImage(named: "hidePasswordIcon")!.withRenderingMode(UIImage.RenderingMode.alwaysTemplate), for: .normal)
        btnPasswordCheck.tintColor = borderColorandThemeColor
        
        btnConfirmPasswordCheck.setImage(UIImage(named: "hidePasswordIcon")!.withRenderingMode(UIImage.RenderingMode.alwaysTemplate), for: .normal)
        btnConfirmPasswordCheck.tintColor = borderColorandThemeColor
   
        btnSignIn.setTitle(Constants.capSignUPButton.localize(), for: .normal)
        ...
        signUpTitleLabel.text = Constants.signUpTitleLabel.localize()
        ...
        lblPassword.text = Constants.passwordLabelPlaceholder.localize()
        lblPasswordErrorMsg.text = Constants.passwordLabelError.localize()
        lblConfirmPassword.text = Constants.confirmPasswordLabel.localize()
        lblConfirmPasswordErrorMsg.text = Constants.confirmPasswordLabelError.localize()
...
//        if let  datePicker = self.txtEnterDOB.inputView as? UIDatePicker {
//            let dateFormatter = DateFormatter()
//            dateFormatter.dateStyle = .medium
//            self.txtEnterDOB.text = dateFormatter.string(from: datePicker.date)
//        }
//        lblEnterDOB.isHidden = false
//        lblEnterDOB.textColor = borderColorandThemeColor
//        txtEnterDOB.setSelectedTextFieldBorderColor()
//        self.txtEnterDOB.resignFirstResponder()
//     }
    ...
    func customViewStyleMethod()
    {
        ...
        else
        {
            ...
            txtPassword.setRightPaddingPoints(50.0)
            txtConfirmPassword.setRightPaddingPoints(50.0)
        }
    }
    var calledFlag = true
    ...
    func hideLable(){
        ...
        lblPasswordErrorMsg.isHidden = true
        lblConfirmPassword.isHidden = true
        lblConfirmPasswordErrorMsg.isHidden = true
        ...
        btnPasswordCheck.isHidden = true
        btnConfirmPasswordCheck.isHidden = true
    }
    
    func setFontAndTextColor() {
        // Set font and text color
        self.btnSignIn.titleLabel?.font                = globalBtnFont
        self.btnSignIn.backgroundColor                 = borderColorandThemeColor
        ...
        self.lblPasswordErrorMsg.textColor             = borderColorandThemeColor
        self.txtPassword.font                          = globalTextFieldFont
        self.txtPassword.textColor                     = textFieldTextColor
        self.lblConfirmPassword.textColor              = borderColorandThemeColor
        self.lblConfirmPasswordErrorMsg.textColor      = borderColorandThemeColor
        self.txtConfirmPassword.font                   = globalTextFieldFont
        self.txtConfirmPassword.textColor              = textFieldTextColor
        ...
        txtPassword.attributedPlaceholder = NSAttributedString(string: Constants.passwordLabelPlaceholder.localize(), attributes: [NSAttributedString.Key.foregroundColor: textPlaceHolder])
        ...
        self.lblPasswordErrorMsg.font               = lblTittleFont
        self.lblConfirmPassword.font                = lblTittleFont
        self.lblConfirmPasswordErrorMsg.font        = lblTittleFont
    }
    ...
    func validateSingleField(textField:UITextField) -> Bool{
        ...
        if(textField == txtPassword) {
            if !isValidPassword(txtPass: txtPassword.text ?? "") {
            // self.alert(message: kKeyEmptyPasswordMessage)
                if (txtPassword.text!.count < 6) {
                    lblPassword.isHidden = false
                    lblPasswordErrorMsg.isHidden = false
                    lblPassword.textColor = errorBorderColor
                    lblPasswordErrorMsg.textColor = errorBorderColor
                    txtPassword.setSelectedTextFieldErrorColor()
                    lblPasswordErrorMsg.text = Constants.passwordCharLimitError.localize()
                     return false
                } else {
                    lblPassword.isHidden = false
                    lblPasswordErrorMsg.isHidden = false
                    ...
                    lblPasswordErrorMsg.textColor = errorBorderColor
                    txtPassword.setSelectedTextFieldErrorColor()
                    lblPasswordErrorMsg.text = Constants.passwordContainOneDigitLimitError.localize()
                    return false
                }
            }
        }
        if(textField == txtConfirmPassword) {
          if !isValidPassword(txtPass: txtConfirmPassword.text ?? "") {
            if (txtPassword.text!.isEmpty) {
                lblConfirmPassword.isHidden = false
                lblConfirmPasswordErrorMsg.isHidden = false
                lblConfirmPassword.textColor = errorBorderColor
                lblConfirmPasswordErrorMsg.textColor = errorBorderColor
                txtConfirmPassword.setSelectedTextFieldErrorColor()
                lblConfirmPasswordErrorMsg.text = Constants.enterConfirmMessageError.localize()
                //self.alert(message: kKeyConfirmPasswordEmptyMessage)
                return false
            } else {
                 return true
            }
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        if textField == txtFirstName {
            ...
            if(textField ==  txtPassword) {
                if (newString.length > 0) {
                    btnPasswordCheck.isHidden = false
                    lblPassword.isHidden = false
                    lblPasswordErrorMsg.isHidden = true
                    lblPassword.textColor = borderColorandThemeColor
                    txtPassword.setSelectedTextFieldBorderColor()
                } else {
                    btnPasswordCheck.isHidden = true
                    lblPassword.isHidden = true
                    lblPasswordErrorMsg.isHidden = true
                    txtPassword.setDefaultBorderColor()
                }
                return count < 40
            }
            ...
            if(textField ==  txtConfirmPassword)
            {
                if (newString.length > 0) {
                    txtPassword.setDefaultBorderColor()
                    btnConfirmPasswordCheck.isHidden = false
                    lblConfirmPassword.isHidden = false
                    lblConfirmPasswordErrorMsg.isHidden = true
                    lblConfirmPassword.textColor = borderColorandThemeColor
    ...
    @IBAction func textFieldEditingFullName(_ sender: FormTextField) {
        if sender == txtFirstName{
            ...
            confirmPasswordString = sender.text!
    ...
    @IBAction func registerButtonAction(_ sender: Any) {
        validateField(completion: { (success) in
                   if success{
                       self.basicRegistrationApiCalling()
                   }
               })
    }
    
    func basicRegistrationApiCalling()
      {
           var finalmobilecode = ""
          ...
          if Connectivity.isConnectedToInternet {
              ...
              let urlString =  kBaseURL + basicreghistrationAPI;
...
//                  param["emergency_contact_number"]  = finalEmergencyMobilecode
                      ...
                      case .success(let dictionary as [String: Any]):
                          if (dictionary["status"] as? Int == 200) {
                              ...
                              DispatchQueue.main.async {
                                  let alertController = UIAlertController(title: Constants.success.localize() , message: Constants.signUpCompletionMessage.localize(), preferredStyle: .alert)
 ...
 var validate : Bool = false
    ...
    func validateField(completion: (_ result: Bool) -> Void){
        ...
        var dob : Bool = true
        var password : Bool = true
        var confirmPassword : Bool = true
        var matchPassword : Bool = true
        if txtFirstName.text == ""{
            lblFirstName.isHidden = false
            lblFirstNameErrorMsg.isHidden = false
            lblFirstName.textColor = errorBorderColor
        ...
        if txtPassword.text != txtConfirmPassword.text {
                ...
                lblConfirmPasswordErrorMsg.textColor = errorBorderColor
                txtConfirmPassword.setSelectedTextFieldErrorColor()
            lblConfirmPasswordErrorMsg.text = Constants.passwordMismatchError.localize()
                matchPassword =  false
        } else {
            matchPassword = true
        }
        if(firstName == true && lastName == true && mobileValidation == true && password == true && confirmPassword == true && matchPassword == true  && city == true && emergencyMobileValidation == true && nationalId == true && gender == true && maritalStatus == true && occupation == true && dob == true){
            validate = true
        } else {
            validate = false
        }
    ...
    @IBAction func btnPasswordShowAction(_ sender: Any) {
        if(eyeIconClick == true) {
            ...
            eyeIconClick = false
//            btnPasswordCheck.setImage(UIImage(named: "showpassIcon"), for: .normal)
             btnPasswordCheck.setImage(UIImage(named: "showpassIcon")!.withRenderingMode(UIImage.RenderingMode.alwaysTemplate), for: .normal)
             btnPasswordCheck.tintColor = borderColorandThemeColor
        } else {
    ...
    @IBAction func btnConfirmPasswordShowAction(_ sender: Any) {
        if(eyeIconClick1 == true) {
            //show password in the textfield box
            txtConfirmPassword.isSecureTextEntry = false
            eyeIconClick1 = false
//            btnConfirmPasswordCheck.setImage(UIImage(named: "showpassIcon"), for: .normal)
            btnConfirmPasswordCheck.setImage(UIImage(named: "showpassIcon")!.withRenderingMode(UIImage.RenderingMode.alwaysTemplate), for: .normal)
                       btnConfirmPasswordCheck.tintColor = borderColorandThemeColor
        } else {
            //Hide password in the textfield box
